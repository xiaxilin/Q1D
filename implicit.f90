...
contains
  ...
  subroutine implicit_solve( cells, faces, dxsi, prim_cc, cons_cc,             &
    ...
    use jacobians,       only : new_jac_vanleer_1D
...
! calculate Jacobians for inflow face
      ...
      call new_jac_vanleer_1D( cons_cc(:,1), cons_cc(:,2), right_jac_L, left_jac_C)

      do cell = 2, cells+1

        call new_jac_vanleer_1D( cons_cc(:,cell), cons_cc(:,cell+1),           &
